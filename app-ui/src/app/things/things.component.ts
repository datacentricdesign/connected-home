...
export class ThingsComponent implements OnInit {
  ...
  myWebSocket: WebSocketSubject<Thing> = webSocket("ws://localhost:80");
      ...
      map((data: Thing[]) => {
        ...
        for (let index = 0; index < data.length; index++) {
          ...
          for (let indexc = 0; indexc < data[index].controls.length; indexc++) {
            data[index].controls_formated.push(
              data[index].controls[indexc].toUpperCase().replace("_", " ")
            );
          }
          data[index].lastUpdateText = moment(
            data[index].last_update * 1000
          ).fromNow();
        }
        return data;
      }),
